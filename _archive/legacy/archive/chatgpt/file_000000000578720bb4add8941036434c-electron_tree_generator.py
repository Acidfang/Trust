...
class ElectronTreeGenerator:
    ...
    def generate_electron_growth_animation(self, filename='electron_growth_animation.gif'):
        ...
        for z in range(1, max_z + 1):
            ...
            for orbital, electrons_count in config.items():
                orbital_type = orbital[-1]
                color = self.orbital_colors.get(orbital_type, '#888888')
                
                # Draw filled electrons
                for e in range(electrons_count):
                    angle = (e / max([2, 6, 10, 14])) * 2 * np.pi
                    radius = float(orbital[0])  # Shell number
                    
                    x = 7 + radius * 1.5 * np.cos(angle)
                    y = 5 + radius * 1.5 * np.sin(angle)
                    
                    circle = Circle((x, y), 0.15, color=color, ec='white',
                ...
                for shell in range(1, max_shell + 1):
                    shell_circle = Circle((7, 5), shell * 1.5,
                                        fill=False, edgecolor='#333333',
                                        linewidth=0.5, linestyle='--', alpha=0.3)
                    ax.add_patch(shell_circle)
            
            # Add element info
            ax.text(7, 9.5, 'Electron Configuration Growth',
                   ha='center', fontsize=12, color='white', weight='bold')
            ax.text(7, 8.8, f'Element: {element_name} (Z = {z})',
                   ha='center', fontsize=11, color='#4ECDC4', weight='bold')
            
            # Show configuration string
            config_str = ''.join([f'{o}{c}' for o, c in config.items()])
            ax.text(7, 0.5, config_str, ha='center', fontsize=9,
        ...
        def animate(frame_num):
            ...
            ax.text(7, 9.5, f'Building Electron Configuration',
                   ha='center', fontsize=12, color='white', weight='bold')
            ax.text(7, 8.8, f'{element_name} (Z={z}, {z} electrons)',
                   ha='center', fontsize=11, color='#4ECDC4', weight='bold')
            
            config_str = ''.join([f'{o}{c}' for o, c in config.items()])
            ax.text(7, 0.5, config_str, ha='center', fontsize=8,
                   color='#888888', family='monospace')
