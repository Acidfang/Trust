#!/usr/bin/env python3
"""
Glow Reasoning Server with detailed error logging
"""
...
def handle_error(error):
    ...
    print(f"[FLASK ERROR HANDLER]\n{error_trace}\n")
    with open("server_error.log", "a") as f:
        f.write(f"\n[EXCEPTION]\n{error_trace}\n")
    return jsonify({"error": str(error), "traceback": error_trace}), 500
...
def handle_query():
    ...
    print(f"\n[QUERY #{counter+1}] Processing: {query[:50]}...")
...
if __name__ == '__main__':
    print("[SERVER] Starting Glow Reasoning Server on port 5555...")
    app.run(debug=False, port=5555, host='localhost', use_reloader=False)
