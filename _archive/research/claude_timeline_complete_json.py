#!/usr/bin/env python3
r"""
COMPLETE CLAUDE MESSAGE TIMELINE GENERATOR (8 JSON exports)

Loads all Claude conversation JSON files from D:\Downloads\Claude
...
class ClaudeTimelineGenerator:
    """Extract timeline from 8 Claude JSON exports."""
    
    def __init__(self, json_dir: str = r"D:\Downloads\Claude"):
    ...
    def load_all_json_exports(self) -> int:
        """Load all 8 JSON files from Claude folder."""
        ...
        print(f"Found {len(json_files)} Claude JSON files to process...")
        
        for idx, json_file in enumerate(json_files):
            try:
                self._load_single_json(json_file)
                print(f"  Processed {idx + 1}/{len(json_files)}: {json_file.name} ({self.conversations_processed} conversations, {len(self.messages)} messages)")
    ...
    def _load_single_json(self, json_file: Path) -> int:
        ...
        try:
            ...
            # Each item in the array IS a message (same as Gemini)
    ...
    def print_preview(self, limit: int = 20):
        ...
        print("\n" + "="*80)
        print("CLAUDE MESSAGE TIMELINE PREVIEW")
        print("="*80)
    ...
    def export_json(self, output_path: str, accountability=None) -> Dict:
        ...
        if accountability:
            description = f"Claude timeline: {len(self.messages)} messages from {self.conversations_processed} conversations"
...
def main():
    ...
    print("\n" + "="*80)
    print("CLAUDE MESSAGE TIMELINE GENERATOR (8 JSON exports)")
    print("="*80)
    
    # [C]: INITIALIZE LIVE ACCOUNTABILITY
    accountability = LiveAccountabilitySystem("accountability.ledger", symbol="claude")
    
    generator = ClaudeTimelineGenerator()
    
    # Load all JSON files
    print("\nLoading all Claude JSON exports...")
    count = generator.load_all_json_exports()
    print(f"\n✓ Extracted {count} total messages from {generator.conversations_processed} conversations")
    ...
    if generator.messages:
        ...
        json_result = generator.export_json("claude_timeline_all_messages.json", accountability=accountability)
        status = "✓" if json_result.get('ledger_recorded') else "✗"
        print(f"{status} JSON timeline: {json_result['path']} [Ledger recorded]")
        
        # Export Text (can also use accountability if desired)
        text_result = generator.export_text("claude_timeline_all_messages.txt")
        print(f"✓ Text timeline: {text_result['path']}")
        
        # Statistics
        stats = generator.get_statistics()
        print("\n" + "-"*80)
        print("CLAUDE TIMELINE STATISTICS")
